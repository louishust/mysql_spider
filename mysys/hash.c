...
static int hashcmp(const HASH *hash, HASH_LINK *pos, const uchar *key,
                   size_t length);
static int hashcmp_with_hash_value(const HASH *hash, HASH_LINK *pos,
                                   my_hash_value_type hash_value,
                                     ...
                                     HASH_SEARCH_STATE *current_record)
{
  ...
  if (hash->records)
  {
    ...
    do
    {
      ...
      if (!hashcmp_with_hash_value(hash,pos,hash_value,key,length))
      {
	...
	*current_record= idx;
	DBUG_RETURN (pos->data);
      ...
      {
	flag=0;					/* Reset flag */
	if (my_hash_mask(pos->hash_value, hash->blength, hash->records) != idx)
	  break;				/* Wrong link */
      }
    }
...
uchar* my_hash_next_with_hash_value(const HASH *hash,
                                    my_hash_value_type hash_value,
                                    const uchar *key, size_t length,
                                    HASH_SEARCH_STATE *current_record)
{
...
{
  my_hash_value_type hash_value = rec_hashnr(info, record);
  return(my_hash_insert_with_hash_value(info, hash_value, record));
}

my_bool my_hash_insert_with_hash_value(HASH *info,
                                       my_hash_value_type hash_value,
                                       const uchar *record)
{
  int flag;
  size_t idx,halfbuff,first_index;
  my_hash_value_type hash_nr,ptr_to_rec_hash_value,ptr_to_rec2_hash_value;
  ...
  if (HASH_UNIQUE & info->flags)
  {
    ...
    if (my_hash_search_using_hash_value(info, hash_value, key, idx))
  ...
  if (idx != info->records)				/* If some records */
  {
	...
	if (!(flag & LOWFIND))
	{
	  if (flag & HIGHFIND)
	  {
	    ...
	    gpos=empty;
	    ptr_to_rec=pos->data;
	    ptr_to_rec_hash_value=pos->hash_value;
	...
	else
	{
	  if (!(flag & LOWUSED))
	  {
	    ...
	    gpos->hash_value=ptr_to_rec_hash_value;
	...
	if (!(flag & HIGHFIND))
	{
	  ...
	  ptr_to_rec2_hash_value=pos->hash_value;
	}
	else
	{
	  if (!(flag & HIGHUSED))
	  {
	    ...
	    gpos2->hash_value=ptr_to_rec2_hash_value;
  ...
  else
  {
    ...
    empty[0]=pos[0];
    gpos= data + my_hash_mask(pos->hash_value, info->blength,
                              info->records + 1);
    if (pos == gpos)
    {
      pos->data=(uchar*) record;
...
my_bool my_hash_delete(HASH *hash, uchar *record)
{
  ...
  hash_value = rec_hashnr(hash, record);
  DBUG_RETURN(my_hash_delete_with_hash_value(hash, hash_value, record));
}

my_bool my_hash_delete_with_hash_value(HASH *hash,
                                       my_hash_value_type hash_value,
                                       uchar *record)
{
  ...
  DBUG_ENTER("my_hash_delete_with_hash_value");
  if (!hash->records)
    DBUG_RETURN(1);
  ...
  else if (pos->next != NO_RECORD)
  {
    ...
    pos->data=empty->data;
    pos->hash_value=empty->hash_value;
    pos->next=empty->next;
  }
  ...
  lastpos_hashnr=lastpos->hash_value;
...
{
  my_hash_value_type hash_value;
  my_hash_value_type old_hash_value;
  DBUG_ENTER("my_hash_update");
  hash_value = rec_hashnr(hash, record);
  old_hash_value = calc_hash(hash, old_key,
                             (old_key_length ?
                              old_key_length :
                              hash->key_length));
  DBUG_RETURN(my_hash_update_with_hash_value(hash, hash_value, record,
                                             old_hash_value,
                                             old_key, old_key_length));
}

my_bool my_hash_update_with_hash_value(HASH *hash,
                                       my_hash_value_type hash_value,
                                       uchar *record,
                                       my_hash_value_type old_hash_value,
                                       uchar *old_key,
                                       size_t old_key_length)
{
  ...
  DBUG_ENTER("my_hash_update_with_hash_value");
  
  if (HASH_UNIQUE & hash->flags)
  {
    HASH_SEARCH_STATE state;
    uchar *found, *new_key= (uchar*) my_hash_key(hash, record, &idx, 1);
    if ((found= my_hash_first_from_hash_value(hash, hash_value, new_key, idx,
                                              &state)))
    {
      do 
      {
      ...
      while ((found= my_hash_next_with_hash_value(hash, hash_value, new_key,
                                                  idx, &state)));
    }
  }
  ...
  idx= my_hash_mask(old_hash_value, blength, records);
  new_index= my_hash_mask(hash_value, blength, records);
  if (idx == new_index)
  ...
  pos=data+new_index;
  new_pos_index= my_hash_mask(pos->hash_value, blength, records);
                     ...
                     uchar *new_row)
{
  if (*current_record != NO_RECORD) {           /* Safety */
    HASH_LINK *pos =
      dynamic_element(&hash->array, *current_record, HASH_LINK*);
    pos->data= new_row;
    pos->hash_value= rec_hashnr(hash, new_row);
  }
}
