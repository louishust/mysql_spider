...
int ha_myisammrg::qcache_insert(Query_cache *qcache,
                                Query_cache_block_table *block_table,
                                TABLE_COUNTER_TYPE &n)
{
  TABLE_LIST *open_table;
  for (open_table= children_l;;
       open_table= open_table->next_global)
  {
    if (!open_table->table->file->qcache_insert(qcache, block_table, n))
      return 0;
    if (&open_table->next_global == children_last_l)
      break;
  }
...
TABLE_COUNTER_TYPE ha_myisammrg::qcache_table_count()
{
  MYRG_INFO *file = myrg_info();
  return (file->end_table - file->open_tables) + 1;
}
