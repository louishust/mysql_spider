    ...
    while (sprit_ptr[0])
    {
      ...
      switch (title_length)
      {
        ...
        case 3:
          ...
          VP_PARAM_INT_WITH_MAX("ait", auto_increment_table, 1,
            share->table_count);
#ifndef WITHOUT_VP_BG_ACCESS
          ...
          VP_PARAM_INT_WITH_MAX("ist", info_src_table, 0, share->table_count);
        ...
        case 19:
          ...
          goto error;
        case 20:
          VP_PARAM_INT_WITH_MAX("auto_increment_table", auto_increment_table,
            1, share->table_count);
          error_num = ER_VP_INVALID_TABLE_INFO_NUM;
          my_printf_error(error_num, ER_VP_INVALID_TABLE_INFO_STR,
            MYF(0), tmp_ptr);
          goto error;
        case 23:
          VP_PARAM_INT_WITH_MAX("infomation_source_table", info_src_table, 0,
            share->table_count);
...
int vp_correspond_columns(
  ...
  if (!share->init || share->reinit)
  {
    ...
    for (roop_count = 0; roop_count < share->table_count;
      roop_count++, tmp_correspond_columns_p++)
    {
    ...
    for (roop_count = 0; roop_count < share->table_count; roop_count++)
    {
      ...
      if (!different_column && part_table_share->fields == table_share->fields)
      {
