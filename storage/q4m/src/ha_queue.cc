...
static HASH allocated_thds;
static pthread_mutex_t allocated_thds_mutex;
static uchar *allocated_thds_get_key(
  THD *thd,
  size_t *length,
...
queue_connection_t *queue_connection_t::current(bool create_if_empty)
{
  ...
  if (conn == NULL && create_if_empty) {
    ...
    if (conn) {
      pthread_mutex_lock(&allocated_thds_mutex);
      if (my_hash_insert(&allocated_thds, (uchar*) current_thd))
      {
        delete conn;
        conn = NULL;
      } else {
        *thd_ha_data(current_thd, queue_hton) = (void *) conn;
        cnt++;
      }
      pthread_mutex_unlock(&allocated_thds_mutex);
    }
  }
  return conn;
}

int queue_connection_t::close_internal(handlerton *hton, THD *thd, bool locked)
{
  if (!locked)
    pthread_mutex_lock(&allocated_thds_mutex);
  queue_connection_t *conn =
    static_cast<queue_connection_t*>(thd_get_ha_data(thd, queue_hton));
  *thd_ha_data(thd, queue_hton) = (void *) NULL;
  if (conn) {
    my_hash_delete(&allocated_thds, (uchar *) thd);
    --cnt;
  }
  if (!locked)
    pthread_mutex_unlock(&allocated_thds_mutex);
...
{
  return close_internal(hton, thd, FALSE);
}
...
static int init_plugin(void *p)
{
  ...
  pthread_mutex_init(&allocated_thds_mutex, MY_MUTEX_INIT_FAST);
  ...
  my_hash_init(&allocated_thds, system_charset_info, 32, 0, 0,
	    reinterpret_cast<my_hash_get_key>(allocated_thds_get_key), 0, 0);
...
static int deinit_plugin(void *p)
{
  ...
  {
    pthread_mutex_lock(&allocated_thds_mutex);
    while ((tmp_thd = (THD *) my_hash_element(&allocated_thds, 0)))
      queue_connection_t::close_internal(queue_hton, tmp_thd, TRUE);
    pthread_mutex_unlock(&allocated_thds_mutex);
  }
  ...
  my_hash_free(&allocated_thds);
  my_hash_free(&queue_open_tables);
  pthread_mutex_destroy(&allocated_thds_mutex);
