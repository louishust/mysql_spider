...
int *slave_transaction_retry_errors;
int slave_transaction_retry_error_length = 0;
...
static void print_slave_transaction_retry_errors(void);
...
int init_slave()
{
  ...
  */
  ...
  if (!slave_transaction_retry_error_length)
    print_slave_transaction_retry_errors();
...
static void print_slave_transaction_retry_errors(void)
{
  ...
  */
  const size_t MIN_ROOM= 10;
  DBUG_ENTER("print_slave_transaction_retry_errors");
  DBUG_ASSERT(sizeof(slave_transaction_retry_error_names) > MIN_ROOM);

  if (!slave_transaction_retry_error_length)
  {
    /* purecov: begin tested */
    memcpy(slave_transaction_retry_error_names, STRING_WITH_LEN("OFF"));
    /* purecov: end */
  }
  else
  {
    char *buff= slave_transaction_retry_error_names;
    char *bend= buff + sizeof(slave_transaction_retry_error_names);
    int  i;

    for (i= 0; i < slave_transaction_retry_error_length; i++)
    {
      if (buff + MIN_ROOM >= bend)
        break; /* purecov: tested */
  ...
  DBUG_PRINT("init", ("error_names: '%s'", slave_transaction_retry_error_names));
  DBUG_VOID_RETURN;
}

void init_slave_transaction_retry_errors(const char* arg)
{
  const char *p;
  long err_code;
  DBUG_ENTER("init_slave_transaction_retry_errors");

  slave_transaction_retry_error_length= 0;
  for (;my_isspace(system_charset_info,*arg);++arg)
    /* empty */;
  for (p= arg; *p; )
  {
  ...
  if (slave_transaction_retry_error_length)
  {
    if (!(slave_transaction_retry_errors= (int *) my_malloc(
      sizeof(int) * slave_transaction_retry_error_length, MYF(MY_WME))))
    {
      fprintf(stderr,
    ...
    for (p= arg, i= 0; *p; i++)
    {
      if (!(p= str2int(p, 10, 0, LONG_MAX, &err_code)))
        break;
      slave_transaction_retry_errors[i]= (int) err_code;
      while (!my_isdigit(system_charset_info,*p) && *p)
        p++;
    }
  } else
    slave_transaction_retry_errors= NULL;
  /* Convert slave transaction retry errors into a printable string. */
  print_slave_transaction_retry_errors();
  DBUG_VOID_RETURN;
}
...
static int has_temporary_error(THD *thd)
{
  ...
  */
  ...
  for (i= 0; i < slave_transaction_retry_error_length; i++)
  {
    if (thd->stmt_da->sql_errno() == slave_transaction_retry_errors[i])
      DBUG_RETURN(1);
  }
        ...
        */
        if (rli->trans_retries < slave_trans_retries)
        {
          ...
          else
          {
            ...
            slave_sleep(thd, max(min(rli->trans_retries, MAX_SLAVE_RETRY_PAUSE), slave_trans_retry_interval),
