...
static Sys_var_uint Sys_xid_cache_split_num(
       "xid_cache_split_num",
       "The number of splitting xid cache for scaling",
       READ_ONLY GLOBAL_VAR(opt_xid_cache_split_num), CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(1, UINT_MAX), DEFAULT(20), BLOCK_SIZE(1));
...
static Sys_var_charptr Sys_slave_transaction_retry_errors(
       "slave_transaction_retry_errors", "Tells the slave thread to retry transaction "
       "for replication when a query event returns an error from the "
       "provided list",
