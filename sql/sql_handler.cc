...
static void mysql_ha_close_childs(THD *thd, TABLE_LIST *tables)
{
  TABLE_LIST *table_list;
  DBUG_ENTER("mysql_ha_close_childs");
  for (table_list = tables->next_global; table_list;
    table_list = table_list->next_global)
  {
    DBUG_PRINT("info",("table_name %s.%s", table_list->table->s->db.str,
      table_list->table->s->table_name.str));
    if (table_list->parent_l == tables)
    {
      DBUG_PRINT("info",("find child"));
      TABLE *table = table_list->table;
      mysql_ha_close_childs(thd, table_list);
...
static void mysql_ha_close_table(THD *thd, TABLE_LIST *tables)
{
  ...
  if (table && !table->s->tmp_table)
  {
    ...
    mysql_ha_close_childs(thd, tables);
    table->open_by_handler= 0;
    (void) close_thread_table(thd, &table);
...
bool mysql_ha_open(THD *thd, TABLE_LIST *tables, bool reopen)
{
  ...
  */
  ...
  for (TABLE_LIST *table_list= hash_tables; table_list;
    table_list= table_list->next_global)
  {
                                  ...
                                  table_list->mdl_request.ticket))
    {
      ...
      bool error2= thd->mdl_context.clone_ticket(&table_list->mdl_request);
    ...
    table_list= table_list->next_global)
  {
    ...
    {
      DBUG_PRINT("info",("set_lock_duration"));
