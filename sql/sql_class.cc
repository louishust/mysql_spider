...
bool xid_cache_init()
{
  ...
  if (!my_multi_malloc(MYF(MY_WME | MY_ZEROFILL),
                       &LOCK_xid_cache,
                       sizeof(mysql_mutex_t) * opt_xid_cache_split_num,
                       &xid_cache,
                       sizeof(HASH) * opt_xid_cache_split_num,
                       NullS))
    return TRUE;

  for (uint j= 0; j < opt_xid_cache_split_num; ++j)
  {
  mysql_mutex_init(key_LOCK_xid_cache, &LOCK_xid_cache[j], MY_MUTEX_INIT_FAST);
  if (my_hash_init(&xid_cache[j], &my_charset_bin, 100, 0, 0,
                   xid_get_hash_key, xid_free_hash, 0))
    return TRUE;
  }
  return FALSE;
}
...
void xid_cache_free()
{
  if (LOCK_xid_cache)
  {
    for (uint j= 0; j < opt_xid_cache_split_num; ++j)
    {
  if (my_hash_inited(&xid_cache[j]))
  {
    my_hash_free(&xid_cache[j]);
    mysql_mutex_destroy(&LOCK_xid_cache[j]);
  }
    }
    my_free(LOCK_xid_cache);
    LOCK_xid_cache= NULL;
  }
}
...
{
  my_hash_value_type hash_value = my_calc_hash(&xid_cache[0],
    (uchar*) xid->key(), xid->key_length());
  uint idx= hash_value % opt_xid_cache_split_num;
  mysql_mutex_lock(&LOCK_xid_cache[idx]);
  XID_STATE *res=(XID_STATE *)my_hash_search_using_hash_value(&xid_cache[idx],
    hash_value, xid->key(), xid->key_length());
  mysql_mutex_unlock(&LOCK_xid_cache[idx]);
  return res;
}
...
bool xid_cache_insert(XID *xid, enum xa_states xa_state)
{
  ...
  uint idx= hash_value % opt_xid_cache_split_num;
  mysql_mutex_lock(&LOCK_xid_cache[idx]);
  if (my_hash_search_using_hash_value(&xid_cache[idx],
    hash_value, xid->key(), xid->key_length()))
    res=0;
  ...
  else
  {
    ...
    xs->in_thd=0;
    xs->rm_error=0;
    res=my_hash_insert(&xid_cache[idx], (uchar*)xs);
  }
  mysql_mutex_unlock(&LOCK_xid_cache[idx]);
  return res;
}
...
bool xid_cache_insert(XID_STATE *xid_state)
{
  ...
  uint idx= hash_value % opt_xid_cache_split_num;
  mysql_mutex_lock(&LOCK_xid_cache[idx]);
  if (my_hash_search_using_hash_value(&xid_cache[idx], hash_value,
  ...
  bool res= my_hash_insert(&xid_cache[idx], (uchar*)xid_state);
  mysql_mutex_unlock(&LOCK_xid_cache[idx]);
  return res;
}
...
void xid_cache_delete(XID_STATE *xid_state)
{
  ...
  uint idx= hash_value % opt_xid_cache_split_num;
  mysql_mutex_lock(&LOCK_xid_cache[idx]);
  my_hash_delete_with_hash_value(&xid_cache[idx], hash_value,
    (uchar *)xid_state);
  mysql_mutex_unlock(&LOCK_xid_cache[idx]);
}
